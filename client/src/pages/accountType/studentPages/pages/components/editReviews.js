...
function EditReviews()
{
    ...
    const handleSubmitNewData = (event) =>
    {
        ...
        Axios.put(`http://localhost:3001/studentHome/editReviews?commentId=${editId}&userId=${userId}`,
        {
            newRating: newRating,
            newReview: newReview,
        }).then((response) =>
    ...
    const handleDeleteReview = (event) =>
    {
        ...
        Axios.delete(`http://localhost:3001/studentHome/editReviews?commentId=${editId}`)
            .then((response) =>
            {
                setEditId(null);
            })
